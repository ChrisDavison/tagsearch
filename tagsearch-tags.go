...
func fileTags(filename string) TagSet {
...
func tagsForFiles(files []string) (map[string][]string, []string) {
	file_tags := make(map[string]TagSet)
	file_tags_strings := make(map[string][]string)

	all_tag_set := make(map[string]bool)

	for _, filename := range files {
		ft := fileTags(filename)
		if len(ft) == 0 {
			continue
		}
		file_tags[filename] = ft
		var ft_strings []string
		for t := range ft {
			ft_strings = append(ft_strings, t)
			all_tag_set[t] = true
		}
		ft_strings_sorted := sort.StringSlice(ft_strings)
		ft_strings_sorted.Sort()
		file_tags_strings[filename] = ft_strings_sorted
	}
